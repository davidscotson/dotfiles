...
set -l __fish_yadm_subcommands -a (yadm introspect commands)
set -p __fish_yadm_subcommands add
...
function __fish_yadm_known_files
    yadm list
end
# Borrowed from git completion file
...
complete -f -c yadm -n __fish_yadm_needs_command -a add -d 'add files to yadm'
...
complete -f -c yadm -n "__fish_yadm_using_command add" -x -a '(__fish_yadm_known_files)'
